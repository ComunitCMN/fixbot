...
#: Без похожих друг на друга символов: код иногда переписывают руками.
...
def new_code(length: int = 8) -> str:
    ...
    Собираем из своего алфавита, а не режем token_urlsafe: там попадаются
    дефисы и подчёркивания, после удаления которых длина плавала, и код
    выходил короче задуманного.
    """
...
ASK = {
    ...
    "amo_token": (
        ...
        "4. Права — <b>сделки, контакты, компании</b>: просмотр, создание, "
        "редактирование. Удаление не нужно.\n"
        "❗️ Область доступа выберите <b>«Все»</b>, а не «Только свои» — "
        "иначе я увижу лишь часть базы и буду считать чужих клиентов новыми.\n"
